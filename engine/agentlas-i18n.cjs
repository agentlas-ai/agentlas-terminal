"use strict";
/*
 * Tiny i18n for the agentlas terminal chrome. Language is chosen in the first-run
 * onboarding wizard and stored in cli-prefs.json. Missing keys fall back to English.
 * Technical tokens (/slash commands, runtime/model names, "runtime"/"perm"/"cwd") stay as-is.
 */

const LANGS = [
  { code: "en", label: "English" },
  { code: "ko", label: "한국어" },
];

const STRINGS = {
  en: {
    // banner
    "banner.help": "for commands",
    "banner.quit": "to quit",
    "banner.interrupt": "to interrupt",
    "banner.product": "Agent OS terminal",
    "banner.session": "%s · %s · %s",
    "banner.location": "%s · / commands · Shift-Tab permissions",
    "banner.menu.help": "Command reference",
    "banner.menu.sessions": "Running sessions",
    "banner.menu.chats": "Past conversations",
    "banner.menu.quit": "Quit",
    "status.title": "Session",
    "status.runtime": "runtime",
    "status.model": "model",
    "status.effort": "effort",
    "status.agent": "agent",
    "status.directory": "directory",
    "status.permission": "permission",
    // picker
    "picker.agents": "Agents",
    "picker.companies": "Companies",
    "picker.none": "(none yet — use /search <task>, /install <slug>, or /import <path>)",
    "picker.prompt": "pick a number/name, or type a task to auto-route (/help, /import <path>, /exit) › ",
    "picker.hint": "Just type — Agentlas routes your task to the right agent. /agents to choose one, / for commands.",
    "composer.autoroute": "auto-route",
    "composer.hint": "/ for commands",
    "picker.noNum": "no agent at that number",
    "picker.noFirm": "no such company",
    "picker.noMatch": 'no match for "%s" — try a number or a name',
    // repl
    "interrupted": "interrupted",
    "cancelling": "cancelling — waiting for child processes to stop",
    "cancellationUnverified": "cancellation requested, but child-process shutdown could not be verified",
    "ctrlcAgain": "Press Ctrl-C again to exit.",
    "bye": "Goodbye.",
    "stalled": "stream stalled — interrupted (idle timeout)",
    "thinkingWith": "thinking with %s",
    "spinnerStop": "ctrl-c to stop",
    "runtime.thinking": "thinking…",
    "runtime.reasoning": "reasoning…",
    "runtime.starting": "starting %s…",
    "runtime.failed": "failed to run %s: %s",
    "runtime.exited": "%s exited with code %s",
    "runtime.noOutput": "%s returned no output",
    "runtime.rateLimit": "%s rate limit reached",
    "runtime.doctor": "Runtime Doctor: %s",
    "runtime.doctorRetry": "Repair complete — retrying the same request once.",
    "noKey": "No %s API key. Add one in the app (Settings → BYOK), or switch with /runtime.",
    "runtimeSet": "runtime → %s",
    "runtimeNotInstalled": "%s CLI is not installed.",
    "runtimeUnknown": "Unknown runtime: %s (claude-code|codex|gemini)",
    "runtimeUnavailable": "No runtime is available. Install a CLI (claude/codex/gemini) or configure an API key/Ollama in the app.",
    "runtimeUsage": "usage: /runtime claude-code|codex|gemini|anthropic|openai|google|ollama|upstage",
    "modelOnlyApi": "/model only applies to BYOK/Ollama (api) runtimes.",
    "modelSet": "model → %s",
    "modelCurrent": "current model: %s",
    "modelDefault": "(default)",
    "effortSet": "effort → %s",
    "effortCurrent": "current effort: %s",
    "effortUsage": "usage: /effort low|medium|high|max|off|auto",
    "permSet": "permission → %s",
    "permUsage": "usage: /permission read|write|full",
    "legacyNetworkUsage": "usage: /legacy-network <request>",
    "stormUsage": "usage: /storm <goal> [--research]",
    "buildUsage": "usage: /build <agent-or-team request>",
    "routeUsage": "usage: /route <request> [--json]",
    "researchUsage": "usage: /research status|gather|search|read|plan [args]",
    "commandQuoteError": "unclosed quote — close it and try again",
    "searchUsage": "usage: /search <what you need>",
    "networkUsage": "usage: /network <request> [--parallel N] [--benchmark]",
    "browserUsage": "usage: /browser <url-or-query>",
    "connectUsage": "usage: /connect <status|telegram|subcommand>",
    "swarmUsage": "usage: /swarm <goal> [--parallel N]",
    "permCurrent": "current permission: %s",
    "permCycleHint": "Shift-Tab to cycle",
    "permCycleConfirm": "permission → %s",
    "permFullArm": "FULL ACCESS IS UNRESTRICTED — press Shift-Tab again within 5 seconds to confirm",
    "permFullCommandArm": "FULL ACCESS IS UNRESTRICTED — run /permission full again within 5 seconds to confirm",
    "permFullConfirm": "FULL ACCESS ON FOR THIS SESSION — approvals and sandboxing are bypassed",
    "sideUsage": "usage: /side <question>",
    "sideNeedsSubject": "pick an agent first, or type the task normally so Agentlas can auto-route it",
    "sideStart": "side question; this answer will not be saved to chat context",
    "sideDone": "side question complete; main chat context unchanged",
    "sideCancelled": "side question cancelled; main chat context unchanged",
    "sideFailed": "side question failed; main chat context unchanged",
    "noAgent": "no agent: %s",
    "noCompany": "no company: %s",
    "agentUsage": "usage: /agent <name>",
    "firmUsage": "usage: /firm <name>",
    "importUsage": "usage: /import <folder>",
    "installUsage": "usage: /install <slug>   (browse in the app, or /marketplace)",
    "installing": "installing %s…",
    "cloudInstalled": "installed: %s",
    "market.title": "Marketplace",
    "market.help": "Install agents or teams from the Hub or a local folder.",
    "market.installHint": "install a Hub agent",
    "market.importHint": "import a local agent/team",
    "market.loggedIn": "credential present — validity unverified",
    "market.loggedOut": "credential not found — sign in via app or set AGENTLAS_SESSION",
    "mcp.none": "(no MCP servers configured — add them in the app)",
    "mcp.playwright": "Playwright browser",
    "mcp.fullOnly": "full only",
    "mcp.wired": "%s reviewed server(s) available only in full-access turns",
    "mcp.usage": "add/remove MCP servers in the app; read/write never inject external MCP tools",
    "mcp.commandUsage": "usage: /mcp",
    "help.mcp": "list configured MCP servers and which are wired",
    "help.project": "inspect or explicitly initialize local project state",
    "resume.title": "Resume — recent sessions",
    "resume.none": "(no saved sessions yet — they're saved after each turn)",
    "resume.usage": "usage: /resume <number>",
    "resume.noNum": "no session at that number",
    "resume.ok": "resumed → %s",
    "help.resume": "list and resume a recent runtime session",
    "cwdNoPath": "no such path: %s",
    "cwdSet": "cwd → %s",
    "noMemory": "(no memory yet)",
    "imported": "imported: %s (%s)",
    "updated": "updated: %s (%s)",
    "switched": "→ %s",
    "unknownCmd": "unknown command: /%s. Type / for the command list.",
    "team.title": "Team — agent → runtime (auto-routed by capability)",
    "team.auto": "auto",
    "team.pinned": "pinned",
    "team.usage": "usage: /team   ·   /team <agent> <claude-code|codex|gemini|auto>",
    "team.set": "%s → %s",
    "routedImage": "routed to %s for image support",
    "judge.source.llm": "judged by the connected model",
    "judge.source.fallback": "deterministic fallback (no connected-model verdict)",
    "usageBar": "session usage",
    "config.title": "Engine auto engagement — explicit on/off (default: off)",
    "config.storm": "Stormbreaker auto-engage on direct-routed real work",
    "config.network": "Workforce Ontology auto-staffing on direct-routed real work",
    "config.usage": "usage: /config storm|network on|off   ·   explicit /storm and /network always work",
    "config.set": "%s → %s",
    "config.externalChange": "preferences changed in another Agentlas terminal: %s",
    "config.autoEngage": "auto-engaging %s (opted in via /config — turn off with /config %s off)",
    "help.config": "explicit on/off for Stormbreaker/Workforce Ontology auto engagement",
    "guard.imageWarn": "this agent works with images, but %s can't generate them — try /runtime gemini (or codex), or /team to pin one",
    "multimodal.title": "Multimodal fallback",
    "multimodal.set": "multimodal %s → %s",
    "multimodal.usage": "set with /multimodal set <image|video|audio> <provider-id>",
    // help rows
    "help.talk": "talk to the current agent/company — streaming + tools",
    "help.help": "show commands, shortcuts, and common flows",
    "help.title": "Help",
    "help.intro": "Run local agents with explicit runtime, permission, files, shell, and history controls.",
    "help.commands": "Commands",
    "skills.title": "Skills",
    "permissions.title": "Permissions",
    "permissions.current": "Current",
    "help.skills": "list slash-command skills",
    "help.agents": "list installed agents",
    "help.team": "view/assign each agent's LLM (auto-routed by capability)",
    "help.agent": "switch to another agent",
    "help.firms": "list companies / switch to one",
    "help.runtime": "switch runtime (claude-code|codex|gemini|anthropic|openai|google|ollama|upstage)",
    "help.model": "set the model (claude/codex/gemini alias or BYOK/Ollama id)",
    "help.effort": "set reasoning effort (low|medium|high|max|off|auto)",
    "help.permission": "what it may do (read|write|full)",
    "help.permissions": "show permission choices and current level",
    "help.setup": "run language, runtime, and permission setup again",
    "help.cwd": "show or change the working folder",
    "help.memory": "show the memory being injected",
    "help.careerGraph": "show/add source refs for the Career Graph routing index",
    "help.ontology": "turn on/list/add project ontology sources with short natural commands",
    "help.side": "ask a side question without saving it to chat context",
    "help.status": "show model/runtime, agent, permission, and directory",
    "help.multimodal": "show/set image, video, and audio fallback providers",
    "help.import": "import a local folder (agent or team)",
    "help.market": "browse/install marketplace agents",
    "help.clear": "clear the chat and redraw",
    "help.storm": "run a force-robust Stormbreaker pipeline on a goal",
    "help.build": "build/repair/package an agent or team (Hephaestus)",
    "help.route": "preview which agent/pipeline would take a request",
    "help.research": "run the Research Engine (search/gather/read)",
    "help.search": "discover agents in the Hub",
    "help.install": "install an agent from the Hub by slug",
    "help.network": "let the host LLM staff and execute an ontology-grounded exact-release task force",
    "help.legacyNetwork": "run the explicit compatibility-only Hephaestus network route",
    "help.browser": "real browser execution hardpoint",
    "help.connect": "wire Telegram / platforms to an agent team",
    "help.swarm": "fan out an emergent agent swarm on a goal",
    "help.doctor": "check runtimes and data",
    "help.compact": "drop older transcript turns and keep the latest context",
    "help.keybindings": "show terminal shortcuts",
    "help.exit": "quit (Ctrl-C: stop the turn / quit when idle)",
    "help.talkKey": "(type a message)",
    // input affordances + commands (history/cost/diff/@file/!shell/multiline/tab)
    "noHistory": "(no history yet)",
    "noCost": "(no usage recorded yet)",
    "cost.title": "Session usage — by runtime",
    "cost.total": "total",
    "diffNoGit": "not a git repository (cwd)",
    "diffClean": "(working tree clean)",
    "help.cost": "session usage/cost, broken down by runtime",
    "help.diff": "show the working-tree git diff",
    "help.history": "list recent inputs (persisted across sessions)",
    "help.tipsTitle": "Shortcuts",
    "help.slash": "start a slash command such as /help or /status",
    "help.atfile": "attach a file's contents to your message",
    "help.bang": "run a shell command in the working folder",
    "help.multiline": "continue input on the next line",
    "help.tab": "autocomplete commands, agents, runtimes, paths",
    "help.arrows": "browse persisted input history",
    "help.ctrlc": "interrupt a running turn, or press twice when idle to quit",
    "help.shiftTab": "cycle read → write → full permission",
    "help.ctrlT": "show or hide the real runtime task/todo list during a turn",
    "palette.title": "Slash commands",
    "palette.search": "type to search",
    "palette.category": "category: %s",
    "palette.examples": "examples: %s",
    "palette.controls": "↑↓ move · Enter run · Tab complete · Esc close",
    "tasks.title": "Tasks",
    "tasks.show": "Ctrl-T show",
    "tasks.hide": "Ctrl-T hide",
    "tasks.pending": "pending",
    "tasks.progress": "in progress",
    "tasks.done": "done",
    "tasks.failed": "failed",
    "category.Help": "Help",
    "category.Session": "Session",
    "category.Discovery": "Discovery",
    "category.Knowledge": "Knowledge",
    "category.Routing": "Routing",
    "category.Settings": "Settings",
    "category.Files": "Files",
    "category.Context": "Context",
    "category.Engine": "Engine",
    "category.Hub": "Hub",
    "category.Health": "Health",
    "compact.noop": "context is already compact (%s message(s))",
    "compact.done": "compacted context: %s → %s message(s)",
    // onboarding wizard
    "wiz.welcome": "Welcome to Agentlas — let's set things up.",
    "wiz.langQ": "Choose your language",
    "wiz.runtimeQ": "Which AI should run your agents by default?",
    "wiz.runtimeAuto": "Use the app's active runtime",
    "wiz.runtimeInstalled": "installed",
    "wiz.runtimeMissing": "not installed",
    "wiz.permQ": "How much should agents be allowed to do by default?",
    "wiz.permRead": "read — inspect only; workspace writes blocked",
    "wiz.permWrite": "write — workspace + runtime temp writes; external MCP off (recommended)",
    "wiz.permFull": "full — bypass approvals and sandbox; trusted use only",
    "wiz.nav": "↑↓ move · Enter select · or type a number",
    "wiz.pick": "choice › ",
    "wiz.default": "(Enter default)",
    "wiz.invalid": "Choose 1–%s.",
    "wiz.saved": "All set. You can change any of this later with /runtime, /permission.",
    "wiz.saveFailed": "Could not save your choices (%s) — they apply to this session only, and setup will ask again next launch.",
    "wiz.changeLang": "Tip: re-run setup anytime with  agentlas setup",
  },
  ko: {
    "banner.help": "명령 보기",
    "banner.quit": "종료",
    "banner.interrupt": "턴 중단",
    "banner.product": "Agent OS 터미널",
    "banner.session": "%s · %s · %s",
    "banner.location": "%s · / 명령 · Shift-Tab 권한",
    "banner.menu.help": "명령 보기",
    "banner.menu.sessions": "실행 중인 세션",
    "banner.menu.chats": "지난 대화",
    "banner.menu.quit": "종료",
    "status.title": "세션",
    "status.runtime": "런타임",
    "status.model": "모델",
    "status.effort": "추론 강도",
    "status.agent": "에이전트",
    "status.directory": "작업 폴더",
    "status.permission": "권한",
    "picker.agents": "에이전트",
    "picker.companies": "회사",
    "picker.none": "(아직 없음 — /search <할 일>, /install <식별자>, /import <경로>를 사용하세요)",
    "picker.prompt": "번호/이름을 고르거나, 할 일을 바로 입력하면 자동 라우팅합니다 (/help, /import <경로>, /exit) › ",
    "picker.hint": "그냥 입력하세요 — 할 일을 알맞은 에이전트로 자동 라우팅합니다. /agents 로 직접 선택, / 로 명령.",
    "composer.autoroute": "자동 라우팅",
    "composer.hint": "/ 명령",
    "picker.noNum": "그 번호의 에이전트가 없습니다",
    "picker.noFirm": "그런 회사가 없습니다",
    "picker.noMatch": '"%s"에 맞는 항목이 없습니다 — 번호나 이름으로',
    "interrupted": "중단됨",
    "cancelling": "취소 중 — 하위 프로세스 종료 확인 중",
    "cancellationUnverified": "취소를 요청했지만 하위 프로세스 종료를 확인하지 못했습니다",
    "ctrlcAgain": "종료하려면 Ctrl-C를 한 번 더 누르세요.",
    "bye": "안녕",
    "stalled": "응답이 지연되어 중단했습니다 (idle timeout)",
    "thinkingWith": "%s로 생각 중",
    "spinnerStop": "ctrl-c로 중단",
    "runtime.thinking": "생각 중…",
    "runtime.reasoning": "추론 중…",
    "runtime.starting": "%s 시작 중…",
    "runtime.failed": "%s 실행 실패: %s",
    "runtime.exited": "%s 종료 코드 %s",
    "runtime.noOutput": "%s가 출력 없이 종료됨",
    "runtime.rateLimit": "%s 사용량 한도에 도달함",
    "runtime.doctor": "런타임 진단: %s",
    "runtime.doctorRetry": "자동 수리 완료 — 같은 요청을 한 번 다시 시도합니다.",
    "noKey": "%s API 키가 없습니다. 앱 설정 → BYOK에서 등록하거나 /runtime으로 전환하세요.",
    "runtimeSet": "런타임 → %s",
    "runtimeNotInstalled": "%s CLI가 설치돼 있지 않습니다.",
    "runtimeUnknown": "알 수 없는 런타임: %s (claude-code|codex|gemini)",
    "runtimeUnavailable": "사용 가능한 런타임이 없습니다. CLI(claude/codex/gemini)를 설치하거나 앱에서 API 키/Ollama를 설정하세요.",
    "runtimeUsage": "사용법: /runtime claude-code|codex|gemini|anthropic|openai|google|ollama|upstage",
    "modelOnlyApi": "/model은 BYOK/Ollama(api) 런타임에서만 적용됩니다.",
    "modelSet": "모델 → %s",
    "modelCurrent": "현재 모델: %s",
    "modelDefault": "(기본값)",
    "effortSet": "추론 강도 → %s",
    "effortCurrent": "현재 추론 강도: %s",
    "effortUsage": "사용법: /effort low|medium|high|max|off|auto",
    "permSet": "권한 → %s",
    "permUsage": "사용법: /permission read|write|full",
    "legacyNetworkUsage": "사용법: /legacy-network <요청>",
    "stormUsage": "사용법: /storm <목표> [--research]",
    "buildUsage": "사용법: /build <만들 에이전트·팀 설명>",
    "routeUsage": "사용법: /route <요청> [--json]",
    "researchUsage": "사용법: /research status|gather|search|read|plan [인자]",
    "commandQuoteError": "따옴표가 닫히지 않았습니다 — 닫은 뒤 다시 시도하세요",
    "searchUsage": "사용법: /search <필요한 에이전트 설명>",
    "networkUsage": "사용법: /network <요청> [--parallel N] [--benchmark]",
    "browserUsage": "사용법: /browser <URL 또는 검색어>",
    "connectUsage": "사용법: /connect <status|telegram|하위 명령>",
    "swarmUsage": "사용법: /swarm <목표> [--parallel N]",
    "permCurrent": "현재 권한: %s",
    "permCycleHint": "Shift-Tab으로 변경",
    "permCycleConfirm": "권한 → %s",
    "permFullArm": "무제한 권한은 승인과 샌드박스를 우회합니다 — 5초 안에 Shift-Tab을 다시 눌러 확인",
    "permFullCommandArm": "무제한 권한은 승인과 샌드박스를 우회합니다 — 5초 안에 /permission full을 다시 실행해 확인",
    "permFullConfirm": "이 세션에서만 무제한 권한 켜짐 — 승인과 샌드박스를 우회함",
    "sideUsage": "사용법: /side <질문>",
    "sideNeedsSubject": "먼저 에이전트를 고르거나, 그냥 할 일을 입력해서 Agentlas가 자동 라우팅하게 하세요.",
    "sideStart": "사이드 질문입니다. 이 답변은 메인 대화 맥락에 저장하지 않습니다.",
    "sideDone": "사이드 질문 완료. 메인 대화 맥락은 그대로입니다.",
    "sideCancelled": "사이드 질문이 중단되었습니다. 메인 대화 맥락은 그대로입니다.",
    "sideFailed": "사이드 질문이 실패했습니다. 메인 대화 맥락은 그대로입니다.",
    "noAgent": "에이전트 없음: %s",
    "noCompany": "회사 없음: %s",
    "agentUsage": "사용법: /agent <이름>",
    "firmUsage": "사용법: /firm <이름>",
    "importUsage": "사용법: /import <폴더>",
    "installUsage": "사용법: /install <식별자>   (마켓플레이스는 앱에서 보거나 /marketplace)",
    "installing": "%s 설치 중…",
    "cloudInstalled": "설치됨: %s",
    "market.title": "마켓플레이스",
    "market.help": "Hub 또는 로컬 폴더에서 에이전트/팀을 설치합니다.",
    "market.installHint": "Hub 에이전트 설치",
    "market.importHint": "로컬 에이전트/팀 가져오기",
    "market.loggedIn": "자격정보 있음 — 유효성 미확인",
    "market.loggedOut": "자격정보 없음 — 앱에서 로그인 또는 AGENTLAS_SESSION 설정",
    "mcp.none": "(설정된 MCP 서버 없음 — 앱에서 추가)",
    "mcp.playwright": "Playwright 브라우저",
    "mcp.fullOnly": "full 전용",
    "mcp.wired": "검토된 서버 %s개를 무제한 권한 턴에서만 사용 가능",
    "mcp.usage": "MCP 서버 추가/삭제는 앱에서; read/write에는 외부 MCP 도구를 주입하지 않음",
    "mcp.commandUsage": "사용법: /mcp",
    "help.mcp": "설정된 MCP 서버와 연결 상태 보기",
    "help.project": "로컬 프로젝트 상태 확인/명시적 초기화",
    "resume.title": "이어하기 — 최근 세션",
    "resume.none": "(저장된 세션 없음 — 각 턴 후 저장됩니다)",
    "resume.usage": "사용법: /resume <번호>",
    "resume.noNum": "그 번호의 세션이 없습니다",
    "resume.ok": "이어하기 → %s",
    "help.resume": "최근 런타임 세션 목록/이어하기",
    "cwdNoPath": "경로 없음: %s",
    "cwdSet": "작업 폴더 → %s",
    "noMemory": "(메모리 없음)",
    "imported": "임포트됨: %s (%s)",
    "updated": "갱신됨: %s (%s)",
    "switched": "→ %s",
    "unknownCmd": "알 수 없는 명령: /%s. / 를 입력하면 명령 목록이 열립니다.",
    "team.title": "팀 — 에이전트 → 런타임 (능력 기반 자동 라우팅)",
    "team.auto": "자동",
    "team.pinned": "고정",
    "team.usage": "사용법: /team   ·   /team <에이전트> <claude-code|codex|gemini|auto>",
    "team.set": "%s → %s",
    "routedImage": "이미지 지원을 위해 %s로 라우팅",
    "judge.source.llm": "판정: 연결 모델",
    "judge.source.fallback": "판정: 결정적 폴백(연결 모델 없음)",
    "usageBar": "세션 사용량",
    "config.title": "엔진 자동 개입 — 명시적 on/off (기본: off)",
    "config.storm": "직답 라우팅된 실작업에 Stormbreaker 자동 개입",
    "config.network": "직답 라우팅된 실작업에 Workforce Ontology 자동 선발",
    "config.usage": "사용법: /config storm|network on|off   ·   /storm·/network 명시 실행은 항상 가능",
    "config.set": "%s → %s",
    "config.externalChange": "다른 Agentlas 터미널의 설정 변경을 반영했습니다: %s",
    "config.autoEngage": "%s 자동 개입 (/config로 켜져 있음 — 해제: /config %s off)",
    "help.config": "Stormbreaker/Workforce Ontology 자동 개입 명시적 on/off",
    "guard.imageWarn": "이 에이전트는 이미지 작업인데 %s는 이미지 생성 불가 — /runtime gemini(또는 codex), 또는 /team으로 고정",
    "multimodal.title": "멀티모달 대체 제공자",
    "multimodal.set": "멀티모달 %s → %s",
    "multimodal.usage": "/multimodal set <image|video|audio> <provider-id> 로 변경",
    "help.talk": "현재 에이전트/회사와 대화 — 스트리밍 + 툴",
    "help.help": "명령·단축키·주요 사용법 보기",
    "help.title": "도움말",
    "help.intro": "런타임·권한·파일·셸·기록을 직접 통제하며 로컬 에이전트를 실행합니다.",
    "help.commands": "명령",
    "skills.title": "스킬",
    "permissions.title": "권한",
    "permissions.current": "현재",
    "help.skills": "슬래시 명령 스킬 목록",
    "help.agents": "설치된 에이전트 목록",
    "help.team": "에이전트별 LLM 보기/지정 (능력 기반 자동)",
    "help.agent": "다른 에이전트로 전환",
    "help.firms": "회사 목록 / 전환",
    "help.runtime": "런타임 전환 (claude-code|codex|gemini|anthropic|openai|google|ollama|upstage)",
    "help.model": "모델 지정 (claude/codex/gemini 별칭 또는 BYOK/Ollama id)",
    "help.effort": "추론 강도 지정 (low|medium|high|max|off|auto)",
    "help.permission": "허용 범위 (read|write|full)",
    "help.permissions": "권한 선택지와 현재 단계 보기",
    "help.setup": "언어·런타임·권한 설정 다시 실행",
    "help.cwd": "작업 폴더 보기/변경",
    "help.memory": "주입되는 메모리 보기",
    "help.careerGraph": "경력 그래프 라우팅 색인의 원본 경로 보기/등록",
    "help.ontology": "짧은 자연어로 프로젝트 온톨로지 켜기/목록/자료 등록",
    "help.side": "메인 대화 맥락에 저장하지 않는 사이드 질문",
    "help.status": "모델/런타임, 에이전트, 권한, 작업 폴더 보기",
    "help.multimodal": "이미지·영상·음성 대체 제공자 보기/변경",
    "help.import": "로컬 폴더(에이전트/팀) 임포트",
    "help.market": "마켓플레이스 에이전트 보기/설치",
    "help.clear": "대화 비우고 다시 그리기",
    "help.storm": "목표를 Stormbreaker 견고 파이프라인으로 실행",
    "help.build": "에이전트/팀 빌드·수리·패키징 (Hephaestus)",
    "help.route": "요청이 어떤 에이전트로 라우팅되는지 미리보기",
    "help.research": "Research Engine 실행 (search/gather/read)",
    "help.search": "Hub에서 에이전트 발견",
    "help.install": "식별자로 Hub 에이전트 설치",
    "help.network": "상위 LLM이 온톨로지에서 정확한 릴리스 TF를 선발·실행",
    "help.legacyNetwork": "호환성 전용 Hephaestus 네트워크 경로를 명시적으로 실행",
    "help.browser": "실제 브라우저 실행 하드포인트",
    "help.connect": "Telegram/플랫폼을 에이전트 팀에 연결",
    "help.swarm": "목표에 창발형 에이전트 스웜 전개",
    "help.doctor": "런타임/데이터 점검",
    "help.compact": "오래된 대화 맥락을 줄이고 최근 맥락 유지",
    "help.keybindings": "터미널 단축키 보기",
    "help.exit": "종료 (Ctrl-C: 턴 중단 / 유휴 시 종료)",
    "help.talkKey": "(메시지 입력)",
    "noHistory": "(아직 기록 없음)",
    "noCost": "(아직 기록된 사용량 없음)",
    "cost.title": "세션 사용량 — 런타임별",
    "cost.total": "합계",
    "diffNoGit": "git 저장소가 아닙니다 (cwd)",
    "diffClean": "(변경 사항 없음)",
    "help.cost": "런타임별 세션 사용량/비용",
    "help.diff": "작업 트리 git diff 보기",
    "help.history": "최근 입력 목록 (세션 간 유지)",
    "help.tipsTitle": "단축키",
    "help.slash": "/help, /status 같은 슬래시 명령 시작",
    "help.atfile": "파일 내용을 메시지에 첨부",
    "help.bang": "작업 폴더에서 셸 명령 실행",
    "help.multiline": "다음 줄에 이어서 입력",
    "help.tab": "명령·에이전트·런타임·경로 자동완성",
    "help.arrows": "세션 간 저장된 입력 기록 탐색",
    "help.ctrlc": "실행 중인 턴 중단, 유휴 시 두 번 누르면 종료",
    "help.shiftTab": "read → write → full 권한 순환",
    "help.ctrlT": "실행 중 실제 런타임 작업 목록 열기/접기",
    "palette.title": "슬래시 명령",
    "palette.search": "입력해서 검색",
    "palette.category": "분류: %s",
    "palette.examples": "예시: %s",
    "palette.controls": "↑↓ 이동 · Enter 실행 · Tab 완성 · Esc 닫기",
    "tasks.title": "작업",
    "tasks.show": "Ctrl-T 열기",
    "tasks.hide": "Ctrl-T 접기",
    "tasks.pending": "대기",
    "tasks.progress": "진행 중",
    "tasks.done": "완료",
    "tasks.failed": "실패",
    "category.Help": "도움말",
    "category.Session": "세션",
    "category.Discovery": "탐색",
    "category.Knowledge": "지식",
    "category.Routing": "라우팅",
    "category.Settings": "설정",
    "category.Files": "파일",
    "category.Context": "맥락",
    "category.Engine": "엔진",
    "category.Hub": "허브",
    "category.Health": "점검",
    "compact.noop": "이미 충분히 compact 상태입니다 (%s개 메시지)",
    "compact.done": "맥락 compact 완료: %s → %s개 메시지",
    "wiz.welcome": "Agentlas에 오신 걸 환영합니다 — 먼저 설정할게요.",
    "wiz.langQ": "언어를 선택하세요",
    "wiz.runtimeQ": "기본으로 어떤 AI가 에이전트를 실행할까요?",
    "wiz.runtimeAuto": "앱의 활성 런타임 사용",
    "wiz.runtimeInstalled": "설치됨",
    "wiz.runtimeMissing": "미설치",
    "wiz.permQ": "에이전트가 기본적으로 어디까지 할 수 있게 할까요?",
    "wiz.permRead": "read — 조회만; 작업 공간 쓰기 차단",
    "wiz.permWrite": "write — 작업 공간·런타임 임시 경로 쓰기; 외부 MCP 꺼짐 (권장)",
    "wiz.permFull": "full — 승인과 샌드박스 우회; 신뢰 환경 전용",
    "wiz.nav": "↑↓ 이동 · Enter 선택 · 또는 번호 입력",
    "wiz.pick": "선택 › ",
    "wiz.default": "(Enter 기본값)",
    "wiz.invalid": "1–%s 중 고르세요.",
    "wiz.saved": "완료. 나중에 /runtime, /permission으로 언제든 바꿀 수 있어요.",
    "wiz.saveFailed": "선택을 저장하지 못했습니다 (%s) — 이번 세션에만 적용되고, 다음 실행 때 다시 물어봅니다.",
    "wiz.changeLang": "팁: 언제든  agentlas setup  으로 다시 설정",
  },
};

function t(lang, key, ...args) {
  const table = STRINGS[lang] || STRINGS.en;
  let s = table[key];
  if (s == null) s = STRINGS.en[key];
  if (s == null) return key;
  let i = 0;
  return s.replace(/%s/g, () => (i < args.length ? String(args[i++]) : "%s"));
}

module.exports = { t, LANGS, STRINGS };
